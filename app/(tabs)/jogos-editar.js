...
export default function JogosEditarScreen() {
  ...
  function selecionarJogos(jogos) {
    setSelecionado(jogos);
    setTitulo(jogos.title ?? "");
    setImagemUrl(jogos.imageUrl ?? "");
    setGenero(jogos.genero ?? "");
    setAnoLancamento(jogos.ano_lancamento ?? "");
    setPlataforma(jogos.plataforma ?? "");
    setDesenvolvedora(jogos.desenvolvedora ?? "");
  }
      ...
      <ScrollView contentContainerStyle={styles.conteudo}>
        ...
        {!selecionado && (
          <>
            ...
            {!carregando &&
              jogos.map((item) => (
                <Pressable key={item.id} style={styles.linha} onPress={() => selecionarJogos(item)}>
